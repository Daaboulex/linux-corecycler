...
class TestDesktopNotify:
    ...
    def test_success_returns_true_and_passes_args(self):
        with patch("notify.shutil.which", return_value="/usr/bin/notify-send"), \
                patch("notify.subprocess.run", return_value=MagicMock(returncode=0)) as run:
    ...
    def test_nonzero_exit_returns_false(self):
        with patch("notify.shutil.which", return_value="/usr/bin/notify-send"), \
                patch("notify.subprocess.run", return_value=MagicMock(returncode=1)):
            assert desktop_notify("t", "b") is False

    def test_subprocess_error_swallowed(self):
        with patch("notify.shutil.which", return_value="/usr/bin/notify-send"), \
                patch("notify.subprocess.run", side_effect=OSError("boom")):
            assert desktop_notify("t", "b") is False
